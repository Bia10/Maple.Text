...
/// <summary>
/// Tests for <see cref="MapleTextStripper"/> branches not reached by the primary test suite,
/// including the ArrayPool path, SkipEntity colon variants, SkipLetterToken paths,
/// and the unknown-code fallback in StripMarkupCore.
/// </summary>
public sealed class MapleTextStripperBranchTests
{
    ...
    // ── SkipEntity: digits reaching EOF (p >= input.Length exit condition) ────

    [Test]
    public async Task StripMarkup_EntityDigitsAtEof_EntirelyConsumed()
    {
        // "#t123" — entity code 't' + digits reaching EOF.
        // Digit loop exits via p >= input.Length; no closing '#' → entire token stripped.
        string result = MapleTextStripper.StripMarkup("#t123");
        await Assert.That(result).IsEqualTo(string.Empty);
    }

    [Test]
    public async Task StripMarkup_EntityDigitsFollowedByNonHash_SkipsOnlyToken()
    {
        // "#t123done" — entity code + digits stop at 'd' (not '#', not ':').
        // Line 161: p < input.Length TRUE, input[p]!='#' → returns p (no +1).
        string result = MapleTextStripper.StripMarkup("#t123done");
        await Assert.That(result).IsEqualTo("done");
    }
    ...
    [Test]
    public async Task StripMarkup_ClientToken_TerminatedByCr_LeavesCrAndRest()
    {
        // "#hname\rrest" — SkipLongForm stops at '\r', leaving '\rrest' in output.
        string result = MapleTextStripper.StripMarkup("#hname\rrest");
        await Assert.That(result).IsEqualTo("\rrest");
    }

    [Test]
    public async Task StripMarkup_ClientToken_TerminatedByLf_LeavesLfAndRest()
    {
        // "#hname\nrest" — SkipLongForm stops at '\n', leaving '\nrest' in output.
        string result = MapleTextStripper.StripMarkup("#hname\nrest");
